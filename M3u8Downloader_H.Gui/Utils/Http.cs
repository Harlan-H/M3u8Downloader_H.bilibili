...
namespace M3u8Downloader_H.Utils
{
    ...
    {
        private readonly ProxyService proxyService = new();
        ...
        {
            if(string.IsNullOrWhiteSpace(proxyService.Address))
                _webProxy = null;
            else
            {
                var webproxy = new WebProxy(proxyService.Address);
                if (!string.IsNullOrWhiteSpace(proxyService.UserName))
                    webproxy.Credentials = new NetworkCredential(proxyService.UserName, proxyService.PassWord);
